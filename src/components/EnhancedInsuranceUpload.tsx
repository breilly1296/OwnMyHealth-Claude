...
import { Upload, X, FileText, Shield, DollarSign, Loader2, CheckCircle, AlertCircle, Eye, Brain, Tag, Search, Filter } from 'lucide-react';
import type { InsurancePlan } from '../types';
import { parseInsuranceDocument, type DocumentParsingResult, type ExtractedInsuranceData } from '../utils/documentParser';
...
export default function EnhancedInsuranceUpload({ isOpen, onClose, onPlanExtracted }: EnhancedInsuranceUploadProps) {
  ...
  const handleImportPlan = (extractedData: ExtractedInsuranceData) => {
    ...
    const plan: InsurancePlan = {
      ...
      planType: (extractedData.planInformation?.planType as any) || 'Other',
      ...
      costs: extractedData.costs?.map(cost => ({
        id: crypto.randomUUID(),
        type: cost.type === 'out_of_pocket_max' ? 'Out-of-Pocket Maximum' : 
              cost.type.charAt(0).toUpperCase() + cost.type.slice(1).replace('_', ' ') as any,
        amount: cost.amount || 0,
        frequency: cost.frequency as any,
        description: cost.description,
        appliesTo: cost.category.charAt(0).toUpperCase() + cost.category.slice(1) as any
      })) || [],
      limitations: extractedData.limitations?.map(limitation => ({
        id: crypto.randomUUID(),
        category: limitation.service,
        description: limitation.description,
        limitType: limitation.type.charAt(0).toUpperCase() + limitation.type.slice(1).replace('_', ' ') as any,
  ...
  const filteredKeyTerms = (terms: any[]) => {
    if (!terms) return [];
