...
import type { ExtractedInsuranceData, ExtractedBenefit, ExtractedCost } from './documentParser';
...
export class InsuranceKnowledgeBase {
  ...
  private normalizeExtractedData(data: ExtractedInsuranceData, sourceFile: string): NormalizedInsurancePlan {
    ...
    return {
      ...
      planType: (data.planInformation?.planType as any) || 'Other',
  ...
  private normalizeExtractedCoverage(coverage: any): NormalizedCoverage {
    let costStructure: CostStructure;
  ...
  private normalizeNetwork(network: any): NormalizedNetwork {
    return {
  ...
  private normalizeExtractedNetwork(network: any): NormalizedNetwork {
    return {
  ...
  private normalizeLimitations(limitations: any[]): NormalizedLimitation[] {
    return limitations?.map(limitation => ({
      id: limitation.id,
      serviceCode: 'GENERAL',
      limitationType: limitation.limitType?.toLowerCase().replace(' ', '_') as any || 'annual',
  ...
  private normalizeExtractedLimitations(limitations: any[]): NormalizedLimitation[] {
    return limitations?.map(limitation => ({
      id: crypto.randomUUID(),
      serviceCode: 'GENERAL',
  ...
  private calculatePlanMetrics(plan: InsurancePlan): PlanMetrics {
    return {
      overallCostRating: 7,
