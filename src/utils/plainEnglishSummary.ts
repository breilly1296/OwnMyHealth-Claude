...
// Risk level explanations in plain English
const RISK_EXPLANATIONS = {
  low: {
    description: 'Your result is in a healthy range',
    urgency: 'No immediate action needed',
    tone: 'reassuring'
  },
  moderate: {
    description: 'Your result is outside the ideal range but not dangerous',
    urgency: 'Worth discussing with your doctor at your next visit',
    tone: 'cautious'
  },
  high: {
    description: 'Your result indicates a significant health concern',
    urgency: 'You should contact your doctor soon to discuss this',
    tone: 'concerning'
  },
  critical: {
    description: 'Your result requires immediate medical attention',
    urgency: 'Contact your doctor or seek medical care right away',
    tone: 'urgent'
  }
};
...
export function generatePlainEnglishSummary(
  ...
  const riskInfo = RISK_EXPLANATIONS[riskAssessment.riskLevel];
...
function generateGenericSummary(biomarker: Biomarker, riskAssessment: RiskAssessment): PlainEnglishSummary {
  const isHigh = biomarker.value > biomarker.normalRange.max;
  const isLow = biomarker.value < biomarker.normalRange.min;
