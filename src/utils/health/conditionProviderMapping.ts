...
// Threshold interface for biomarker evaluation
interface BiomarkerThreshold {
  operator: '<' | '>=' | 'outside';
  value?: number;
  range?: [number, number];
}
...
// Helper functions
function calculateDeviation(biomarker: Biomarker, threshold: BiomarkerThreshold): number {
  if (threshold.operator === '<') {
