...
|---|---|---|---|---|
| Security Management Process — Risk Analysis | §164.308(a)(1)(ii)(A) | ⏳ | No formal risk analysis doc in repo. Live code-level audits exist in `New Project Documents/SECURITY_AUDIT_*.md` (see [`SECURITY_STATUS.md`](./SECURITY_STATUS.md) for consolidated findings). | Produce a signed HIPAA Risk Analysis before beta. Owner: TBD (external). |
| Security Management — Risk Management (remediation) | §164.308(a)(1)(ii)(B) | 🟡 | [`SECURITY_STATUS.md`](./SECURITY_STATUS.md) tracks C-1…C-8 open findings; PR #30 closed C-1/F-14/F-15 on 2026-04-16. | Runtime RLS gap (C-8) still open — see §4 Technical Safeguards. |
...
| Security Incident Procedures | §164.308(a)(6)(ii) | 🟡 | [`RUNBOOK.md`](./RUNBOOK.md) contains operational incident runbook; no HIPAA-specific breach-response SOP. | Cross-reference breach-notification SOP (see §5). |
| Contingency Plan — Data Backup | §164.308(a)(7)(ii)(A) | 🟡 | Cloud SQL automated backups (GCP default). No in-repo restore-test evidence. | Schedule quarterly restore drill; document in [`RUNBOOK.md`](./RUNBOOK.md). |
| Contingency Plan — Disaster Recovery | §164.308(a)(7)(ii)(B) | 🟡 | Infra is redeployable from `.github/workflows/deploy.yml`; DB restore via Cloud SQL PITR. | Document RPO/RTO targets. |
| Contingency Plan — Emergency Mode Operation | §164.308(a)(7)(ii)(C) | ⏳ | — | Draft after DR targets set. |
| Contingency Plan — Testing & Revision | §164.308(a)(7)(ii)(D) | ⏳ | — | Annual tabletop exercise. |
| Contingency Plan — Applications & Data Criticality Analysis | §164.308(a)(7)(ii)(E) | ⏳ | — | — |
...
| Access control — technical (RLS at DB layer) | §164.312(a)(1) | ⚠️ **policy present, runtime gap** | Policies defined at [`backend/prisma/migrations/20260107_add_rls_policies/migration.sql:1-L100+`](../backend/prisma/migrations/20260107_add_rls_policies/migration.sql) — `current_user_id()` helper at `:17`, `is_admin_session()` at `:28`, `has_provider_access()` at `:39`, `ALTER TABLE … ENABLE ROW LEVEL SECURITY` on 16 tables at `:L68-L83`. Application-side wrappers `withRLSContext` / `withRLSTransaction` at [`database.ts:456`](../backend/src/services/database.ts) / [`database.ts:477`](../backend/src/services/database.ts) issue parameterized `SELECT set_config('app.current_user_id', …, true)` via `applyRLSContext` at [`database.ts:377-L386`](../backend/src/services/database.ts). Self-check `assertNoBypassRLS()` at [`database.ts:220`](../backend/src/services/database.ts). CI footgun guard at `scripts/check-rls-wrappers.sh` (see docblock at [`database.ts:L14-L31`](../backend/src/services/database.ts)). | **Open finding C-8**: the live application role has `BYPASSRLS=true` in both dev and prod, so policies do not actually enforce. The code landed (PR #30, 2026-04-16 — closes C-1, F-14, F-15) but the infrastructure cutover to a NOBYPASSRLS `omh_app` role is not yet done. See [`SECURITY_STATUS.md`](./SECURITY_STATUS.md) (finding C-8) and the boot-time warning logic at [`database.ts:L247-L269`](../backend/src/services/database.ts). Tracked by env var `RLS_ENFORCEMENT=strict` (when set, boot fails). |
...
Controller → withRLSContext(userId, async (tx) => ...) — database.ts:456
   │                                                  │
   │                                                  └── SET LOCAL app.current_user_id  ⚠️  C-8: BYPASSRLS role defeats policy
   ▼                                                       (database.ts:377, :220)
tx.* query          ──┐
   │                  │
   ▼                  ▼
...
|---|---|---|---|
| Risk Analysis | §164.308(a)(1)(ii)(A) | ⏳ | TBD (external: not yet drafted). Closest proxies are `New Project Documents/SECURITY_AUDIT_*.md` and [`SECURITY_STATUS.md`](./SECURITY_STATUS.md) — code-level, not policy-level. |
| Risk Management Plan | §164.308(a)(1)(ii)(B) | 🟡 | Remediation tracker in [`SECURITY_STATUS.md`](./SECURITY_STATUS.md) (C-1…C-8). |
...
| Security Incident Response SOP | §164.308(a)(6) | 🟡 | Operational runbook at [`RUNBOOK.md`](./RUNBOOK.md); HIPAA-specific SOP pending. |
| Contingency Plan (backup + DR + testing) | §164.308(a)(7) | 🟡 | Cloud SQL backups on; no signed plan. |
| Business Associate Agreements | §164.308(b) / §164.314(a) | 🟡 | Anthropic ✅ 2026-04-16; GCP and SendGrid TBD — see §1. |
| Policies & Procedures (general) | §164.316(a) | ⏳ | No `docs/policies/` folder yet. |
| Documentation retention (6 years) | §164.316(b)(2) | 🟡 | `AuditLog` retains 7y ([`auditLog.ts:9`](../backend/src/services/auditLog.ts)). Policy docs not yet created, so retention N/A until they exist. |
| Breach Notification SOP | §164.400–414 | ⏳ | TBD — see §5. |
...
|---|---|
| Patient sees only their own rows | RLS policies (e.g., `users_select_own` at [`migration.sql:L90-L95`](../backend/prisma/migrations/20260107_add_rls_policies/migration.sql)) + application wrappers `withRLSContext(userId, …)` at [`database.ts:456`](../backend/src/services/database.ts). ⚠️ see C-8. |
...
|---|---|---|---|
| **Pre-beta (now)** | Close C-8; close remaining BAAs; name compliance officer | 1) Cut `omh_app` NOBYPASSRLS role in Cloud SQL + rotate `DATABASE_URL` in Secret Manager; set `RLS_ENFORCEMENT=strict`. 2) Confirm GCP BAA in Console; execute SendGrid BAA. 3) Appoint privacy/security officer. | [`SECURITY_STATUS.md`](./SECURITY_STATUS.md) (C-8); §1 BAA table; §2 admin safeguards |
| **Pre-beta — documentation** | Draft required HIPAA policies | Risk Analysis, Sanction Policy, Workforce Access Authorization SOP, Termination Checklist, Security Awareness Training Plan, Breach Notification SOP, Contingency Plan (RPO/RTO + restore drill). | §6 table |
...
4. **What's the status of §164.312(a)(1) access control and what's the open runtime gap?** ⚠️ policy-present/runtime-gap. RLS SQL exists at [`migration.sql:1-L100+`](../backend/prisma/migrations/20260107_add_rls_policies/migration.sql) and app wrappers at [`database.ts:456`](../backend/src/services/database.ts), but the DB login has `BYPASSRLS=true` — see finding **C-8** in [`SECURITY_STATUS.md`](./SECURITY_STATUS.md). Boot-time warning at [`database.ts:L247-L269`](../backend/src/services/database.ts).
...
10. **What's the RLS runtime gap (C-8) and where is the remediation plan tracked?** App runs as a `BYPASSRLS` DB role in dev and prod, so RLS policies do not enforce. Remediation tracked in [`SECURITY_STATUS.md`](./SECURITY_STATUS.md) (C-8) and in the §8 Roadmap "Pre-beta" row. Cutover: create NOBYPASSRLS `omh_app` role in Cloud SQL, rotate `DATABASE_URL` in Secret Manager, then set `RLS_ENFORCEMENT=strict` so the boot-time check at [`database.ts:L247-L269`](../backend/src/services/database.ts) escalates the warning to a hard fail.
...
- [RUNBOOK.md](./RUNBOOK.md) — operational incident playbook (input for the pending breach-notification SOP).
