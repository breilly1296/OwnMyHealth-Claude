...
## Verified codebase counts (2026-08-01)
...
| Thing | Count | Path |
|---|---|---|
| Route files | 18 (incl. `index.ts`) — unchanged | `backend/src/routes/` |
| Controllers | 10 domain controllers (+ `index.ts`, `testHelpers.ts`); upload logic lives in `controllers/upload/` (`labUploadController`, `sbcUploadController`, `shared`, `index`) — unchanged | `backend/src/controllers/` |
| Services | 27 top-level `.ts` (incl. `index.ts`) plus subdirs `fhir/` (7), `knowledge/` (4), **`storage/` (4 — new: `gcsBackend`, `localBackend`, `keys`, `types`)**, `data/` (1) | `backend/src/services/` |
| Middleware | 10 (+ `index.ts`): `aiSpendGuard`, `auth`, `csrf`, `demoProtection`, `errorHandler`, `planGating`, `rateLimitStore`, `rateLimiter`, `rbac`, `validation` — unchanged | `backend/src/middleware/` |
| Rate limiters | **8** (standard, auth, strictAuth, upload, sensitive, ai, providerAccessRequest, bulkOperation) — unchanged | `backend/src/middleware/rateLimiter.ts` |
| `aiSpendGuard` mount points | **8** across 5 route files (`aiRoutes`, `uploadRoutes` ×3, `biomarkerRoutes`, `expenseRoutes`, `insuranceRoutes` ×2) — unchanged | `backend/src/middleware/aiSpendGuard.ts` |
| **Storage backends** | **2** — `gcs` \| `local`, selected from `config.storage.backend`; `local` is the dev default and refused in prod/staging | `backend/src/services/storageService.ts` |
| Frontend API modules | **18 + `index.ts`** (+`pagination.ts` since 06-16) | `src/services/api/` |
| Frontend `.tsx` files | **75 across 15 component dirs** (+`legal/`) | `src/components/` |
| Frontend hooks | **8 + `index.ts`** (incl. `useFocusTrap`) | `src/hooks/` |
| Prisma models | 19 — unchanged | `backend/prisma/schema.prisma` |
| Prisma migrations | **34 directories** (newest `20260712_add_sessions_update_policy`) | `backend/prisma/migrations/` |
| Encrypted PHI | `PHI_FIELDS` = 14 models / 39 fields, in lockstep with the schema's 39 `*Encrypted` columns — unchanged | `backend/src/services/encryption.ts` |
| Backend test files | **66** `*.test.ts` | `backend/src/` |
| Frontend test files | **33** | `src/` |
| GitHub workflows | **5** (`ci.yml`, `deploy.yml`, `deploy-staging.yml`, `maintenance.yml`, **`secret-history-scan.yml`**) | `.github/workflows/` |
...
| Item | Frequency |
|---|---|
| Counts in this file | Quarterly (run `Glob` across key dirs) — next due **2026-11-01** |
