...
## Files to Review
- `backend/src/middleware/planGating.ts` (the gate — `requirePlanLimit(limitKey)` and its alias `requirePlanFeature(feature)`; reads the live plan from the DB under RLS at `planGating.ts:66`, applies `planExpiresAt` downgrade at `:73`, emits the 403 body at `:90`)
...
- `backend/src/routes/adminRoutes.ts` (`PATCH /users/:id/plan` at `:564` — the ONLY write path to `users.plan` today; gated by `requireRole('ADMIN')` + `blockDemoAdminAccess` at `:30-31`)
- `backend/prisma/schema.prisma` (`User.plan String @default("FREE") @db.VarChar(20)` at `:34`, `planExpiresAt` `:35`, `planUpdatedAt` `:36`. NOTE: this is a **VARCHAR, not a Prisma enum** — the `PlanType` enum at `:531` is HMO/PPO/EPO/POS/HDHP for `InsurancePlan.planType`, a different concept)
...
- `backend/src/middleware/validation.ts` (`schemas.admin.updateUserPlan` at `:782` — `z.enum(['FREE','PRO','TEAM'])` + optional ISO `expiresAt`)
- `backend/src/middleware/auth.ts` (`req.user.plan` populated from the JWT at `:107`/`:157`/`:207`, falling back to `'FREE'` — this is the *stale snapshot* that `planGating` deliberately bypasses)
...
- Gated route files (enumerate what gates what):
  ...
  - `backend/src/routes/biomarkerRoutes.ts:125` — `requirePlanLimit('aiGuidancePerDay')`
  - `backend/src/routes/expenseRoutes.ts:116` — `requirePlanLimit('costAnalysisPerMonth')`
  - `backend/src/routes/insuranceRoutes.ts:124,137` — `requirePlanLimit('pdfUploadsPerMonth')` (reanalyze + upload-sbc)
  - `backend/src/routes/uploadRoutes.ts:82,99,129` — `requirePlanLimit('pdfUploadsPerMonth')` (lab-report, insurance-sbc, lab-results-ocr)
  - `backend/src/routes/settingsRoutes.ts:80` — `requirePlanFeature('healthProfile')` (PATCH only; GET ungated by design)
  - `backend/src/routes/fhirRoutes.ts:34,47` — `requirePlanFeature('questFhirIntegration')` (connect + sync)
...
- `src/services/api/client.ts` (`isPlanLimitError` narrowing helper at `:55`; the `PLAN_LIMIT_EXCEEDED` → `apiError.planLimit` mapping at `:328`)
- `src/services/api/ai.ts` (SSE chat client — re-parses `PLAN_LIMIT_EXCEEDED` off the streaming 4xx body at `:135`)
- `src/components/settings/PlanSection.tsx` (tier badge, usage bars, feature list; the "Upgrade" CTA is a TODO/`onError` placeholder at `:157`)
...
- `backend/src/schedulers/emailScheduler.ts:247-267` (reads `planExpiresAt` for the 7-day expiry-warning email — note it does NOT write/downgrade `plan`; the only enforcement of expiry is at request time in `planGating`)
...
- **Tier source of truth**: `users.plan` (`schema.prisma:34`) — a **VARCHAR(20)
...
- **The gate**: `requirePlanLimit(limitKey)` middleware (`planGating.ts:37`).
  `requirePlanFeature` is a thin alias for the same function (`:120`) used when the
  ...
  subscription falls back to `FREE` limits. On a DB error it logs and **falls back to
  the JWT plan** (`:83`) rather than wedging every gated route on a transient outage.
...
- **Billing reality (the gap)**: there is **no payment processing**. `plans.ts`
  ...
  (`adminRoutes.ts:564`), gated by `requireRole('ADMIN')`. The migration, `plans.ts`,
...
- **Error contract**: the gate fails with **HTTP 403** and a hand-built body
  ...
  and the **503 SERVICE_UNAVAILABLE** spend-cap / BAA refusals (`aiSpendGuard.ts:42`,
  biomarker BAA gate `biomarkerRoutes.ts:144`). The frontend keys off the
...
- [ ] Enumerate the gate→limit mapping and confirm each is present:
  `aiRoutes.ts:34` → `aiChatsPerDay`; `biomarkerRoutes.ts:125` → `aiGuidancePerDay`;
  `expenseRoutes.ts:116` → `costAnalysisPerMonth`; `insuranceRoutes.ts:124,137` and
  `uploadRoutes.ts:82,99,129` → `pdfUploadsPerMonth`; `settingsRoutes.ts:80` →
  `healthProfile`; `fhirRoutes.ts:34,47` → `questFhirIntegration`.
- [ ] KNOWN GAP — `insurancePlans` is **counted and displayed but NEVER enforced**: `POST /api/v1/insurance/plans` (`insuranceRoutes.ts:80`) has no `requirePlanLimit('insurancePlans')`. A FREE user (`insurancePlans:1`) can create unlimited active plans via the API. `usageTracker` counts `activeInsurancePlans` and `PlanSection` shows the bar, but nothing blocks. Confirm and report.
- [ ] KNOWN GAP — `maxBiomarkers` is **counted but NEVER enforced**: `POST /api/v1/biomarkers` (`biomarkerRoutes.ts:83`) and `POST /batch` (`:91`) carry no `requirePlanLimit('maxBiomarkers')`. A FREE user (`maxBiomarkers:50`) can store unlimited biomarkers by direct entry. (The PDF *upload* path is bounded by `pdfUploadsPerMonth`, but manual/batch entry is not.) Confirm and report.
- [ ] KNOWN GAP — `providerSharing` is a paid boolean (`false` on FREE) but **no route calls `requirePlanFeature('providerSharing')`**. Check the patient/provider consent routes (`patientRoutes.ts`, `providerRoutes.ts`) — confirm a FREE user can still create provider-sharing consents server-side. Report whether this is intended (consent is a HIPAA right) or a gating gap, but do not assume.
...
- [ ] `users.plan` can be written by exactly one realized path: the admin endpoint `PATCH /api/v1/admin/users/:id/plan` (route at `adminRoutes.ts:564`; the `tx.user.update` write at `:587`), which is behind `requireRole('ADMIN')` + `blockDemoAdminAccess` (`adminRoutes.ts:30-31`) and runs under `withRLSContext(null, { isAdmin:true })`. Confirm no other writer exists.
- [ ] No self-service endpoint writes `plan`: confirm `settingsController` (PATCH `/settings/profile`, `/notifications`, `/health-profile`) does NOT accept or persist a `plan` field, and that no Zod settings schema includes `plan`. (The only `plan` enum in `validation.ts` is `schemas.admin.updateUserPlan:782` — admin-scoped.) Cross-link [[30-admin-security]] for the prevent-self-role-elevation analogue (same threat class: a user must not be able to raise their own entitlement).
- [ ] The admin plan change is audited with before/after state (`auditService.logUpdate('admin_user_plan', …)` at `adminRoutes.ts:621`) — confirm `previousPlan`/`newPlan` and the acting admin id are captured (see [[05-audit-logging]]).
- [ ] Mass-assignment check: the admin update uses an explicit `data: { plan, planExpiresAt, planUpdatedAt }` (no `...req.body` spread) at `adminRoutes.ts:589` — confirm no controller spreads request body into a `user.update` in a way that could let `plan` ride along from a non-admin route.
- [ ] `req.user.plan` (from the JWT, `auth.ts:107`) is treated as an untrusted stale snapshot: confirm the gate reads the DB (`planGating.ts:66`) and only falls back to the JWT on DB error (`:83`). Assess the fallback: during a DB outage a recently-downgraded user keeps premium for up to the access-token lifetime — is that acceptable, or should the gate fail closed?
...
- [ ] Audit-row counting is correct: `aiChatsPerDay`/`aiGuidancePerDay` count `AuditLog` rows by `resourceType` (`'HealthGuide'` / `'biomarker_ai_guidance'`) + `action='READ'` (`usageTracker.ts:69-91`). Confirm the writer controllers emit exactly these `resourceType` strings (`aiChatController`, `biomarkerRoutes.ts:266`) — a string drift silently under-counts and lets the limit be exceeded. (Cross-check the same sync warned about in [[27-ai-integration]] §8.)
- [ ] Audit-row counting cannot be deflated by failed/blocked calls: confirm a *blocked* AI call (e.g. `GUIDANCE_BLOCKED_NO_BAA`, `biomarkerRoutes.ts:141`) writes a different `operation`/`resourceType` so it isn't miscounted as a successful use — and conversely that a *successful* use is always logged so it IS counted.
...
- [ ] Confirm there is **no payment integration**: grep `stripe`, `webhook`, `billing`, `checkout`, `payment` across `backend/src/` and `src/` — expect only comments/TODOs (`plans.ts:6`, `migration.sql:9`, `planRoutes.ts:9`, `PlanSection.tsx:7,157`). Report that displayed prices (`PLANS[*].price`/`annualPrice`) are decorative and tie to no charge.
- [ ] Document the gap between displayed prices and actual entitlement changes: a user can see "$9.99/mo Pro" but the "Upgrade" button only fires `onError('Upgrades are not available yet…')` (`PlanSection.tsx:157`). There is no way for a user to actually pay for or self-activate a paid tier.
- [ ] Document how `users.plan` is set **today**: manually, via the admin endpoint (`adminRoutes.ts:564`) or a direct DB update (per the `plans.ts:5` comment). Confirm the admin endpoint is the only application path and is properly RBAC-gated.
...
- [ ] `planExpiresAt` enforcement is request-time only (`planGating.ts:73`); no scheduled job downgrades `plan` itself — `emailScheduler.ts:247` only *reads* it to send an expiry warning. Confirm an expired paid user is correctly limited on every gated route (request-time downgrade) even though their `plan` column still says `PRO`/`TEAM`, and that `planRoutes.ts` reporting reflects the effective (expired→FREE) limits or clearly shows the expiry.
...
- [ ] The client maps the body into `apiError.planLimit` (`client.ts:328`) and the SSE chat path re-parses it off the streaming 4xx body (`ai.ts:135`) — confirm both surfaces (REST + SSE) build the same `planLimit` shape so the upgrade CTA renders consistently.
...
- [ ] The "Upgrade" CTA is honest about the unwired billing path: `PlanSection.tsx:157` shows "Upgrades are not available yet. Contact us…" rather than dead-ending in a broken checkout. Confirm no UI element implies a working purchase flow.

### 6. Demo Accounts & Feature-Flag Alignment
- [ ] Demo vs plan interaction: AI routes stack `blockDemoAI` alongside the plan gate (e.g. `aiRoutes.ts:33-34`). Confirm `blockDemoAI` (`demoProtection.ts:164`) returns **403 ForbiddenError** for the demo account *independently* of plan — a demo account must be blocked from AI even if its `plan` column were elevated, and the demo block is a hard 403 (not a separate quota). Confirm ordering doesn't matter for the outcome (either guard refusing is sufficient).
- [ ] Feature flags in `plans.ts` align with what routes actually gate: `questFhirIntegration` (`plans.ts:28`) is enforced at `fhirRoutes.ts:34,47` ✔; `healthProfile` (`plans.ts:25`) at `settingsRoutes.ts:80` ✔. Confirm the FHIR feature flag and the route gate use the same key, and cross-link [[41-fhir-lab-integration]] for the lab-connection specifics. Re-flag the unenforced flags from §1 (`providerSharing`) and the unenforced numeric limits (`insurancePlans`, `maxBiomarkers`).
- [ ] `dataExport` is `true` on every tier by design (`plans.ts:56`, comment: "HIPAA requires this regardless of plan") — confirm the export route (`settingsRoutes.ts:86`) is correctly NOT gated, so a downgraded user can still exercise their HIPAA right of access (see [[29-data-portability]]). This is a *correct* absence of a gate, not a gap.
...
grep -rn "requirePlanLimit('maxBiomarkers')\|requirePlanLimit('insurancePlans')\|requirePlanFeature('providerSharing')" backend/src/  # expect ZERO hits → the known gaps
...
## Questions to Ask
1. Are `maxBiomarkers` and `insurancePlans` *supposed* to be hard limits? They are counted by `usageTracker` and drawn in `PlanSection`, but no route enforces them — is the visible bar a promise the API doesn't keep, or are these intentionally soft/advisory caps?
2. Should `providerSharing` be a server-enforced paid feature? Right now FREE shows it as locked (`plans.ts:55`) but the consent routes don't call `requirePlanFeature('providerSharing')`. Is consent-based sharing a HIPAA right that must stay ungated regardless of tier (like `dataExport`)?
...
7. On a DB error the gate falls back to the stale JWT plan (`planGating.ts:83`) — for a paid feature, is "fail open to the last-known plan" acceptable, or should it fail closed to FREE during a DB outage?
