...
16. **Deployment topology** — diagram + environment breakdown (local, staging, prod) + service-to-service map. **Capture the post-06-01 CI-gated pipeline**: `deploy.yml` invokes `ci.yml` via `workflow_call` (`needs: ci`), so nothing builds/ships unless lint + test + build + gitleaks + the live-PG RLS job pass; migrations run as the `ownmyhealth-migrate` Cloud Run job AFTER image push and BEFORE the revision is staged; the deploy uses a 0%-traffic staged revision → smoke-test → named-revision promote. Also note `maintenance.yml` (manual one-time backfills via the `ownmyhealth-maintenance` job) and, since 2026-07-11, the **`e2e` job in `ci.yml`** (full Playwright suite against real Postgres) plus the nightly `secret-history-scan.yml`. **Posture (2026-07-14):** GCP billing is disabled and there is no deployment target — write this section as the *launch* topology, explicitly labelled as such, not as a description of a running system. See `OPEN_FINDINGS.md` §Posture.
16b. **Storage topology (OF-23)** — `storageService.ts` is a backend-selecting facade over `storage/gcsBackend.ts` and `storage/localBackend.ts`, chosen from `config.storage.backend` at first use (`storageService.ts:33-44`). Diagram both paths. Call out that `local` writes AES-256-GCM-sealed blobs (envelope `['OMHL'|0x01|iv|tag|ciphertext]`) under `config.storage.localDir` using the **master** `PHI_ENCRYPTION_KEY`, is the **development default**, and is **refused in production/staging** (`config/index.ts:349`). Downloads are a backend stream proxy for both backends — there is no signed-URL path.
