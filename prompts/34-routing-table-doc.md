...
| `backend/src/middleware/csrf.ts` | `csrfProtection` middleware + the three exemption lists (`publicAuthRoutes`, `bearerOnlyStreamingRoutes`, scheduler route). |
...
7. **CSRF exemption list** — Bearer-auth'd / streaming / scheduler routes, with justification. There are three exemption buckets in `csrf.ts`: `publicAuthRoutes` (login/register/refresh/etc.), `bearerOnlyStreamingRoutes` (`/ai/chat` — must use `requireBearerAuth`), and the scheduler route (`/internal/audit-cleanup`).
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| POST | `/api/v1/auth/login` | `authRoutes.ts:Lxx` | public | exempt (`publicAuthRoutes`) | `authLimiter` + `strictAuthLimiter` | — | — | — | `schemas.auth.login` | `authController.login:Lxx` | — | yes |
...
| POST | `/api/v1/ai/chat` | `aiRoutes.ts:L29` | yes (`requireBearerAuth`, router-level) | exempt (`bearerOnlyStreamingRoutes`) | `aiLimiter` | — | `requirePlanLimit('aiChatsPerDay')` + `aiSpendGuard` | (see controller) | `schemas.ai.chat` | `aiChatController.handleAIChat` | `blockDemoAI` | — |
| ... | ... | ... | ... | ... | ... | ... | ... | ... | ... | ... | ... | ... |
...
|---|---|---|
| `authenticate` | `backend/src/middleware/auth.ts:L71` | `biomarkerRoutes.ts:L47` (router-level), `onboardingRoutes.ts:L20` (router-level), ... (N uses) |
| `requireBearerAuth` | `backend/src/middleware/auth.ts:L180` | `aiRoutes.ts:L21` (router-level — CSRF-exempt streaming) |
| `aiLimiter` | `backend/src/middleware/rateLimiter.ts:L108` | `aiRoutes.ts:L31`, `biomarkerRoutes.ts:L122` (guidance), `uploadRoutes.ts` (lab-report/sbc/ocr), `expenseRoutes.ts` (analyze), ... |
| `providerAccessRequestLimiter` | `backend/src/middleware/rateLimiter.ts:L133` | `providerRoutes.ts` (access-request fan-out) |
| `aiSpendGuard` | `backend/src/middleware/aiSpendGuard.ts:L23` | `aiRoutes.ts:L32`, `biomarkerRoutes.ts:L123`, ... (all AI routes) |
| `requirePlanLimit` / `requirePlanFeature` | `backend/src/middleware/planGating.ts:L37` / `L120` | `aiRoutes.ts`, `biomarkerRoutes.ts`, `uploadRoutes.ts`, `fhirRoutes.ts`, ... |
| ... | ... | ... |

### CSRF exemption list

Authority: `backend/src/middleware/csrf.ts` `validateCsrfToken` — three explicit lists.

| Route(s) | Bucket | Exemption reason | Authority |
|---|---|---|---|
| `/auth/login`, `/auth/register`, `/auth/demo`, `/auth/refresh`, `/auth/forgot-password`, `/auth/reset-password`, `/auth/verify-email`, `/auth/resend-verification`, `/marketplace/plans/search` | `publicAuthRoutes` | Public/pre-session — no session cookie to protect yet (note: `/marketplace/plans/search` is dead — the CMS Marketplace feature was removed; flag it) | `csrf.ts:L98-108` |
| `POST /ai/chat` | `bearerOnlyStreamingRoutes` | SSE/`EventSource` can't attach `x-csrf-token`; safe ONLY because the route uses `requireBearerAuth` (cookie-auth rejected) | `csrf.ts:L116-118` |
| `POST /internal/audit-cleanup` | scheduler | Cloud Scheduler can't carry the CSRF cookie; authed by constant-time `X-Cleanup-Token`; 404s unless `AUDIT_CLEANUP_TOKEN` set | `csrf.ts:L139` |
...
- `blockDemoAdminAccess` — applied router-level in `adminRoutes.ts:L30` (runs between auth and RBAC).
...
4. Which routes are exempt from CSRF, in which of the three buckets, and why?
...
- **For CSRF exemptions**: read `backend/src/middleware/csrf.ts` — the three lists (`publicAuthRoutes`, `bearerOnlyStreamingRoutes`, scheduler route) are the authoritative exemption set.
