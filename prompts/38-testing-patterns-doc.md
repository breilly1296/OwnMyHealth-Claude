...
| `backend/src/services/*.test.ts` (e.g., `encryption.test.ts`, `authService.test.ts`, `aiCostTracker.test.ts`, `pdfTextExtraction.test.ts`, `claudeExtraction.test.ts`, `sbcExtraction.test.ts`, `database.test.ts`) | Service recipe. |
...
| `backend/src/middleware/*.test.ts` (`errorHandler.test.ts`, `rbac.test.ts`, `validation.test.ts`, `csrf.test.ts`, `rateLimitStore.test.ts`) | Middleware recipe. |
...
1. **Test pyramid** — unit vs integration vs e2e, counts, runtimes, what each catches. As of 2026-06-01: 34 backend `*.test.ts` (colocated), 14 frontend tests under `src/__tests__/`, 5 Playwright e2e specs. Note the live-DB integration tests (`rls.test.ts`) skip when `DATABASE_URL`/`PHI_ENCRYPTION_KEY` are absent.
...
6. **Middleware test recipe** — full example copy (`rbac`, `validation`, `csrf`, `errorHandler`, `rateLimitStore`).
...
11. **Security-domain test recipes** — FHIR SSRF guard (`fhir/urlSafety.test.ts`), AI cost/budget (`aiCostTracker.test.ts`), PHI redaction (`utils/phiRedaction.test.ts`), provider-access uniformity (`providerRoutes.requestUniformity.test.ts`), internal/cron endpoints (`internalRoutes.test.ts`).
12. **Fixture + factory conventions** — how to seed users, biomarkers, insurance plans, etc. (via `testHelpers.ts` + the e2e seed script).
13. **PHI-aware test conventions** — never print decrypted PHI, always use real encryption path.
...
|---|---|---|
| Prisma tx | `createMockPrismaTransaction()` in `controllers/testHelpers.ts` | Returns a mock tx with `findMany/findFirst/findUnique/create/update/delete/count` spies for every model the controllers touch (User, Biomarker, InsurancePlan, InsuranceBenefit, LabConnection, etc.). Inject via the `withRLSContext`/`withRLSTransaction` mock. |
...
- **Test counts**: `Glob pattern: "backend/src/**/*.test.ts"` (currently 34 files, colocated), `Glob pattern: "src/__tests__/**/*.test.{ts,tsx}"` (currently 14 files), `Glob pattern: "e2e/*.spec.ts"` (currently 5 specs). Re-glob for exact numbers.
