...
> **Note:** `STRATEGY.md` and its sibling docs are generated by the doc-prompt family (14-23, 33-40, 46) into `New Project Documents/`. On a clean checkout they may not exist yet (first run = clean-slate write); on a refresh, treat the existing `STRATEGY.md` as a prior version to supersede. Cross-link to siblings as canonical targets, but verify they exist (`Glob "New Project Documents/*.md"`) — if a sibling has not been generated yet, write `(doc pending — see prompt ./NN-foo.md)` per `_doc-quality`.
...
6. **Provider collaboration strategy** — consent model, permission flags, scope of access. Cite `providerRoutes.ts`, `patientRoutes.ts`, `ProviderPatient` model (status enum `ProviderPatientStatus`, relation enum `ProviderRelationType`). The provider-facing UI is now **shipped** — `src/components/provider/CareTeamPage.tsx` and `src/components/provider/MyPatientsPage.tsx`. Do **not** carry the old "no provider UI yet" framing or list the UI gap as an open question; both are obsolete.
...
8. **Strategic decisions log** — major choices with rationale. Pull from commit bodies ("chose X because Y"), memory entries, and `CLAUDE.md`. Include the two post-freeze delivery/release decisions: (a) migrations run as a dedicated Cloud Run job (`ownmyhealth-migrate`) rather than at container boot, and (b) deploys are gated on full CI (`.github/workflows/deploy.yml` `needs: ci`; `backend/Dockerfile` CMD `node dist/app.js`) — both adopted after a 10-day silent prod outage caused by a boot-time guard, and the biomarker-series correctness rework (`biomarkerSeries.ts`) that made trends actually function.
...
|---|---|---|
| Biomarker tracking (manual entry, history, trends, AI guidance) | `backend/src/routes/biomarkerRoutes.ts`, `Biomarker`/`BiomarkerHistory` models, `src/components/biomarkers/`; trends now backed by `backend/src/services/biomarkerSeries.ts` (`upsertBiomarkerReading` — all create/bulk/FHIR paths append to one series; pre-2026-06-13 each write made a disconnected single-point row, so trends were structurally broken) | Shipped |
...
| Provider-patient consent (provider UI: Care Team + My Patients) | `providerRoutes.ts`, `patientRoutes.ts`, `ProviderPatient` model, `src/components/provider/CareTeamPage.tsx`, `src/components/provider/MyPatientsPage.tsx` | Shipped |
| File management (lab reports + OCR) | `fileRoutes.ts`, `uploadRoutes.ts`, `fileController.ts` (list/audited-proxy-stream download/delete — `getFileStream` + `stream.pipe(res)`, no-store; the legacy 15-min GCS signed-URL endpoint was removed as a capture-replay egress vector), `controllers/upload/` (labUploadController/sbcUploadController), `ocrService.ts`, `pdfParser.ts` | Shipped |
...
|---|---|---|
| _(none currently confirmed)_ | — | — |

> **Do NOT list `reminderFrequency` / `ReminderFrequency` as dead code** — it is now wired to a live feature. `emailScheduler.ts` reads `HealthGoal.reminderFrequency` to drive per-goal reminder cadence (DAILY/WEEKLY/BIWEEKLY/MONTHLY via `REMINDER_INTERVAL_DAYS`), and the create/update goal handlers persist it (`backend/src/schedulers/emailScheduler.ts:316,323`; `backend/src/controllers/healthGoalsController.ts:431,531`; `schema.prisma:479,637`). If older `CLAUDE.md`/memory still calls it dead code, treat that as stale.
>
> Do not list DNA/Genetics here either — those models were dropped in migration `20260423_drop_dna_genetics`. Verify the schema before adding any deprecated row; only models present in `schema.prisma` but genuinely unused belong here. If you find none, leave the table empty rather than inventing entries.
...
| Shipped | C-8 BYPASSRLS cutover / FORCE ROW LEVEL SECURITY on all RLS tables + boot hard-fail | migration `20260613_force_rls_and_audit_retention`; `backend/src/services/database.ts:192-193,217,270,303` (`assertNoBypassRLS()` + `assertRLSForced()`) |
| Shipped | Biomarker time-series fix — trends were structurally broken (each write made a disconnected single-point row); rework makes all create/bulk/FHIR paths append to one series | `backend/src/services/biomarkerSeries.ts` (`upsertBiomarkerReading`) |
| Shipped | Delivery rework — migrations run as dedicated Cloud Run job (`ownmyhealth-migrate`), not at container boot; deploys gated on full CI | `backend/Dockerfile` (CMD `node dist/app.js`, no boot migrate); `.github/workflows/deploy.yml` (`needs: ci` + `ownmyhealth-migrate` job) |
